...
void setColor(int i)
{
        switch(i){
          ...
          default:ucg_SetColor(&ucg,0,CBODY);break;  
        }  
}
...
//        ucg_DrawBox(&ucg,0,HHeader,x,yy);     		
	  ...
	  case 2:
	    if (getDdmm())
			sprintf(strdate,"%02d-%02d-%4d", dt->tm_mday, dt->tm_mon+1,  dt->tm_year+1900);
	    else
			sprintf(strdate,"%02d-%02d-%4d", dt->tm_mon+1, dt->tm_mday, dt->tm_year+1900);
...
void namesetUcg(char* ici)
{
	...
	lline[STATIONNAME] = &nameset[strlen(nameNum)];
